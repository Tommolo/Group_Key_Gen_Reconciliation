...
def check_mean_in_interval(channel_coefficients,quantization_intervals):
     mean_interval=0
     for i in range(len(quantization_intervals)):
        if np.mean(channel_coefficients) >= quantization_intervals[i] and np.mean(channel_coefficients) < quantization_intervals[i+1]:
            mean_interval=i
     return mean_interval

def quantization_by_indexes_alice(channel_coefficients,quantization_intervals):
    quantization_by_indexes=[]
    j=check_mean_in_interval(channel_coefficients,quantization_intervals)
    for i in range(0,len(channel_coefficients)):
        if channel_coefficients[i] >= np.mean(channel_coefficients) or channel_coefficients[i] < quantization_intervals[j-1]:
...
def quantization_by_indexes_bob(channel_coefficients,quantization_intervals):
    ...
    for i in range(0,len(channel_coefficients)):
        if channel_coefficients[i] >= quantization_intervals[j+2]  or channel_coefficients[i] < quantization_intervals[j-2]:
           quantization_by_indexes.append(i) 
    return quantization_by_indexes
...
for _ in range(num_iterations):
    ...
    channel_coefficients = generate_channel_coefficients(3000, 0.9, 0.1, 0.1)
